...
ponteiro busca(TLista* lista, char x, ponteiro *antes){ // encontrar x na lista
...
**/
    ponteiro aux, antes;
    aux = busca(lista,x,&antes) // econtrar x
    if(aux==NULL || aux->info > x) { // se x não estiver na lista ou lista vazia
        return 0;
    }
    else {
        /* remove x */
